...
A straight-line path in joint space with trapezoidal time scaling. Motion time is minimized subject to joint velocity and acceleration limits, and the constant-velocity phase absorbs the remaining time after the ramp segments. This is the computationally cheapest option and avoids IK entirely.

### Task-Space: Straight-Line Path with 3rd-Order Polynomial
A straight-line path in SE(3) using matrix exponential interpolation, with a cubic polynomial time scaling. IK is called at each time step to recover the joint trajectory. Start/end poses were chosen to avoid the singular zones identified during the Jacobian analysis.

### Decoupled Task-Space: Separate Position and Orientation
Position was interpolated linearly in Cartesian space and orientation via geodesic interpolation in SO(3), each with its own cubic polynomial time scaling. This gives independent control over how fast the end-effector translates versus rotates, which is useful when the straight-line SE(3) path would sweep through an ill-conditioned region.
