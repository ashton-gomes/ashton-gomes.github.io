...
![Crank Wheel CAD](/assets/crank_wheel.jpg)
*The 6" steel flywheel with dual roller bearings pressed into the wheel face. The dual-bearing interface eliminates cantilevered pin bending under peak damping loads.*
...
![Frame FEA Stress](/assets/frame_fea_stress.jpg)
*Frame von Mises stress plot under 750 lbf at 5° off-axis. Peak stress is localized to the gearbox mount and remains well below yield.*

![Frame FEA FoS](/assets/frame_fea_fos.jpg)
*Frame Factor of Safety distribution. Minimum FoS = 3.1 at the gearbox mounting plate, against a yield strength of 530 MPa.*
...
Because the shock operates as a connecting rod (not a pure linear actuator), the instantaneous force vector rotates as a function of crank angle. The Teensy logs raw encoder counts and load cell voltage simultaneously; a **Python post-processing pipeline** then applies a cosine correction factor derived from the instantaneous crank angle to project all force data back onto the axial reference frame. This ensures the output FV curves reflect true hydraulic damping resistance rather than the apparent force along the crank's line of action.

---

## Results & Impact

- **Structural:** Frame validated to a minimum FoS of **3.1** under worst-case loading; crossbar achieves **5.8 FoS** with only **0.006" deflection**, confirming the load path routes through the sensor rather than the structure.
...
- **DAQ Validation:** Functional load cell + Teensy/ADS1220 bench system already collecting data, confirming signal quality ahead of full system integration.
- **Motivation from real data:** An on-car linear potentiometer and body accelerometer system was used to estimate the actual shock forces experienced during testing — this data drove the 750 lbf worst-case design load and confirmed the velocity range the dyno needs to cover.
...
![Full Assembly CAD](/assets/shock_dyno_render.jpg)
*Complete SolidWorks assembly. The crank-flywheel drive is at the base, with the shock running vertically through the tower structure to the Whiffle Tree load cell interface at the top.*
